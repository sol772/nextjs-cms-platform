...
export interface CommentItem {
    ...
    depth: number;
    m_name?: string;
    c_name?: string; // 유지보수 댓글
    c_reg_date?: string;
    c_wdate?: string; // 유지보수 댓글
    c_contents?: string;
    c_content?: string; // 유지보수 댓글
...
interface CommentProps {
    ...
    maintPage?: boolean; // 유지보수 댓글 페이지인지 여부
}
...
export default function Comment({
    ...
    const [editValue, setEditValue] = useState(item.c_contents || item.c_content || "");
            ...
            >
                <div className="relative z-[2] flex flex-wrap justify-between">
                    <div className="flex items-center gap-[16px]">
                        ...
                        <p className="font-[500]">{item.m_name || item.c_name}</p>
                        ...
                        {item.c_wdate && (
                            <p className="text-[14px] text-[#999]">
                                {format(new Date(item.c_wdate), "yyyy.MM.dd a hh:mm", {
                                    locale: ko,
                                })}
                            </p>
                        )}
                ...
                <div className="p-[8px_0_8px_40px]">
                    {!editOn && <p className="text-[15px] text-[#666]">{item.c_contents || item.c_content}</p>}
                    {/* 댓글 수정 */}
                    {editOn && (
