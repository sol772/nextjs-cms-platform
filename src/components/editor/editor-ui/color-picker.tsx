...
function ColorPickerRootImpl(props: ColorPickerRootImplProps) {
    ...
    const composedRef = useComposedRefs(ref as PossibleRef<HTMLDivElement>, node => setFormTrigger(node));
...
interface ColorPickerFormatSelectProps extends Omit<React.ComponentProps<typeof Select>, "value" | "onValueChange"> {
    className?: string;
}
...
function ColorPickerFormatSelect(props: ColorPickerFormatSelectProps) {
    ...
    return (
        <Select
            ...
            disabled={context.disabled}
        >
            <SelectTrigger data-slot="color-picker-format-select-trigger" className={cn("h-8", className)}>
                <SelectValue />
            </SelectTrigger>
            <SelectContent>
